...
.post('/email_send', async (req, res) => {
    try {
        ...
        if(data.length >= 1) {
            ...
            const [data_] = await dbConnection.execute("INSERT INTO users (uid, password, password_salt, age, email, name, job, academy, grade, token, email_verify) \
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [req.body.uid, hash_pass, pass_salt, req.body.age, req.body.email, req.body.name, req.body.job, JSON.stringify({academy:[]}), req.body.grade, code, 0]);
